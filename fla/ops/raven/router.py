...
def router_reference(router, f, topk, router_score='sigmoid', gumbel=False, seed=None):
    ...
    scores = torch.softmax(router, dim=-1) if router_score == 'softmax' else torch.sigmoid(router)
